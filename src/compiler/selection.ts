...
type ScalarField<
    ...
    C extends TypeContext,
    Name extends string,
> = Name extends "__typename" ? PossibleRuntimeTypes<S, C>
...
type PossibleTypesFromMeta<Meta> = Meta extends
    GraphQLAbstractType<infer Possible> ? Possible
...
type PossibleRuntimeTypes<
    S extends GraphQLSchema,
    C extends TypeContext,
> = [ AbstractPossibleTypes<S, C> ] extends [ never ] ? C["name"]
...
type TakeFieldHead<S extends string> = TakeName<S> extends Match<
...
type WrapRelation<Value, Relation extends GraphQLRelation> =
    Relation["multiple"] extends true ? Relation["nullable"] extends true ?
                | (Relation["itemNullable"] extends true ? (Value | null)[]
                    : Value[])
                | null
        : Relation["itemNullable"] extends true ? (Value | null)[]
        : Value[]
        : Relation["nullable"] extends true ? Value | null
        : Value;

type FieldKey<Alias extends string | undefined, Name extends string> =
    Alias extends string ? Alias : Name;

type SameType<A, B> = [ A ] extends [ B ] ? ([ B ] extends [ A ] ? true : false)
    : false;
...
> = Depth["length"] extends 16 ? GraphQLError<
        "QUERY_TOO_COMPLEX",
...
type FieldConflict<Left, Right, Depth extends unknown[] = []> = Left extends
    FieldResult<
        infer Key,
        infer Value,
        boolean,
    ...
    >
        ...
        : SameType<FieldName, OtherFieldName> extends true
            ? SameArguments<Args, OtherArgs> extends true
                ? SameType<Value, OtherValue> extends true ? never
                : Value extends
                    ObjectFieldValue<infer LeftFields, GraphQLRelation>
...
// One property per response key, holding the union of every FieldResult that
// carries it (key remapping merges duplicate keys into a union). Linear in
// the number of fields.
type FieldsByKey<Fields> = {
    [
        F in Extract<
            Fields,
            FieldResult<string, unknown, boolean, unknown>
        > as F["key"]
    ]: F;
};

// Only the FieldResults whose response key occurs more than once; the
// common no-duplicate selection set never reaches the pairwise check.
type DuplicateKeyFields<Fields> = FieldsByKey<Fields> extends infer ByKey ? {
        [K in keyof ByKey]: true extends IsUnion<ByKey[K]> ? ByKey[K] : never;
    }[keyof ByKey]
    : never;

// Spec §5.3.2 FieldsInSetCanMerge, restricted to keys that actually repeat:
// pairs are compared only inside their own key bucket.
type FieldConflicts<Fields, Depth extends unknown[] = []> =
    Depth["length"] extends 16 ? GraphQLError<
            "QUERY_TOO_COMPLEX",
            "field merge nesting exceeds compiler depth budget"
        >
        : DuplicateKeyFields<Fields> extends infer Duplicates
            ? ConflictsWithin<Duplicates, Duplicates, Depth>
        : never;

// Distributes over the duplicate-key fields; `All` stays the whole set so
// each field is checked against every other carrier of its key.
type ConflictsWithin<Fields, All, Depth extends unknown[]> = Fields extends
    FieldResult<infer Key, unknown, boolean, unknown>
    ? FieldConflict<Fields, Extract<All, { key: Key; }>, Depth>
    : never;
...
> = SameType<Current["namespace"], Target["namespace"]> extends true ? [
        Extract<
            PossibleRuntimeTypes<S, Current>,
            PossibleRuntimeTypes<S, Target>
        >,
    ] extends [ never ] ? false
    : true
    : false;
...
type FragmentIsConditional<
    ...
    Current extends TypeContext,
    Target extends TypeContext,
> = [ PossibleRuntimeTypes<S, Current> ] extends
    [ PossibleRuntimeTypes<S, Target> ] ? false
    : true;
...
type ApplyOptional<Fields, Optional extends boolean> = Optional extends true
                ...
                ? RelationField<S, C, Head["name"]> extends infer Relation
                    ? [ Relation ] extends [ never ]
                        ? ScalarField<S, C, Head["name"]> extends infer Scalar
                            ? [ Scalar ] extends [ never ] ? GraphQLError<
                                    "UNKNOWN_FIELD",
                    ...
                    : Relation extends GraphQLRelation<infer Ref>
                        ? SkipIgnored<Rest> extends `{${string}`
                            ? TakeBraced<Rest> extends Match<
                                ...
                                infer After extends string
                            >
                                ? ResolveType<Ref, C["namespace"]> extends
                                    infer Target extends TypeContext
...
> = Name extends Visited
    ...
    : Extract<Fragments, { name: Name; }> extends infer Fragment
        ...
        >
            ? ResolveType<On, C["namespace"]> extends
                infer Target extends TypeContext
                ? HasType<S, Target> extends true
                    ? FragmentApplies<S, C, Target> extends true ? RunSelection<
                            Selection,
                            S,
                            Target,
                            Fragments,
...
type CompileInlineBody<
    ...
    Steps extends unknown[],
    Depth extends unknown[],
> = TakeDirectives<Source, S, "INLINE_FRAGMENT", C["namespace"]> extends
    DirectivesResult<
    ...
    > ? RunSelection<
            ...
            Visited,
            [ unknown, ...Depth ]
        > extends infer InlineResult ? InlineResult extends SelectionSuccess<
                infer InlineFields,
                infer InlineUses
            > ? CompileSelectionWorker<
                    After,
...
> = SkipIgnored<S0> extends `${"@" | "{"}${string}`
    ...
    : TakeName<S0> extends Match<
        infer Name extends string,
        infer Rest extends string
    > ? Name extends "on" ? TakeName<Rest> extends Match<
                infer On extends string,
            ...
            >
                ? ResolveType<On, C["namespace"]> extends
                    ...
                    : GraphQLError<
                        "UNKNOWN_TYPE",
                        `unknown inline fragment type: ${On}`
                    >
                : never
            : TakeName<Rest>
        : TakeDirectives<Rest, S, "FRAGMENT_SPREAD", C["namespace"]> extends
            DirectivesResult<
    ...
    : SkipIgnored<Source> extends infer Rest extends string
        ? Rest extends ""
            ...
            : FieldConflicts<Fields> extends infer Conflicts
                ...
                : Conflicts
            : never
        : Rest extends `...${infer AfterSpread}` ? CompileSpread<
                AfterSpread,
                S,
                C,
                Fragments,
...
type DriveSelection<R, Chunks extends unknown[] = []> = Chunks["length"] extends
    3 ? GraphQLError<
        "QUERY_TOO_COMPLEX",
        "selection set exceeds compiler width budget"
    >
...
type FieldKeys<Fields> = Fields extends
    FieldResult<infer Key, unknown, boolean, unknown> ? Key : never;

// Every carrier of the same response key whose type condition overlaps this
// occurrence's contributes its nested fields (spec §5.3.2 CollectFields
// merges them for any runtime type both conditions include). Fields from a
// carrier that covers this occurrence's condition are always present; fields
// from a narrower or partially overlapping carrier become optional, the same
// way a `... on User { name }` scalar is optional on the interface.
type MergedNestedFields<All, Key extends string, Condition> = All extends
    FieldResult<
        Key,
        infer Value,
        boolean,
    ...
    : Value extends ObjectFieldValue<infer Nested, GraphQLRelation>
        ? [ Condition ] extends [ OtherCondition ] ? Nested
        : ApplyOptional<Nested, true>
    : never
    : never;

// An occurrence whose type condition is strictly contained in another
// carrier's condition produces a shape that is a subtype of the broader
// carrier's merged shape (its fields appear there as optional), so it adds
// nothing to the result union and is skipped to keep the union minimal.
type IsSubsumed<All, Key extends string, Condition> = All extends FieldResult<
    Key,
    unknown,
    boolean,
    unknown,
    ...
    infer OtherCondition
>
    ? [ Condition ] extends [ OtherCondition ]
        ? [ OtherCondition ] extends [ Condition ] ? false : true
    : false
    : never;

type FieldValue<Fields, Key extends string, All = Fields> = Fields extends
    FieldResult<
        Key,
        infer Value,
        boolean,
    ...
    ? Value extends ObjectFieldValue<unknown, infer Relation>
        ? true extends IsSubsumed<All, Key, Condition> ? never
        : WrapRelation<
...
type FieldUses<Fields> = Fields extends
    FieldResult<string, unknown, boolean, infer Uses> ? Uses
...
type OptionalFlag<Fields, Key extends string> =
    Extract<Fields, { key: Key; }> extends infer Matches
        ? Matches extends { optional: infer Optional extends boolean; }
            ? Optional
        : never
        : never;
...
type RequiredKeys<Fields> = {
    [K in FieldKeys<Fields>]: false extends OptionalFlag<Fields, K> ? K : never;
}[FieldKeys<Fields>];
...
> = IsUnionSource<Source> extends true ? UnionSourceError
    : ResolveType<Root, S["defaultSchema"]> extends
        infer Context extends TypeContext
